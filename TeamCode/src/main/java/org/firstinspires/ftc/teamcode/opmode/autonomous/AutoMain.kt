    ...
    override fun runOpMode() {
        ...
        when (cubePosition) {
            ...
            CubeVision.CubePlacement.Right -> {
                backOffset = pickPark(12.0, 0.0, 0.0)
                when (position) {
                    Position.Back -> {
                        when (team) {
                            Team.Red -> {
                                ...
                                Encoders.driveTo(forward, tiles(1))
                                sleep(100)
                                Encoders.strafeTo(left, tiles(0.25))
                                Hook.moveHook(-0.5)
                                sleep(400)
                                Hook.stop()
                                Encoders.spinTo(-10.0)
